const MODULE_ID = "gluniverse-cargo-grid";
...
function registerSettings() {
  ...
  game.settings.register(MODULE_ID, SETTINGS.playerVisible, {
    ...
    default: false,
    onChange: visible => {
      if (!visible && game.socket) game.socket.emit(SOCKET_NAME, { type: "closePlayers" });
      scheduleRefresh();
    }
  });
...
async function setData(data, { emit = true } = {}) {
  ...
  if (emit && game.socket) game.socket.emit(SOCKET_NAME, { type: "refresh" });
  scheduleRefresh();
}
